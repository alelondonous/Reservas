...
export default function Aulas(props){
   ...
   // var bloqueA = [];
    ...
    const [bloquec, setbloquec] = useState([]);
    //const [bloqued, setbloqued] = useState([]);
    const [bloquee, setbloquee] = useState([]);
    const [bloquechec, setbloquechec] = useState([]);
    const [bloquecasita, setbloquecasita] = useState([]);
    const [bloquearquite, setbloquearquite] = useState([]);
    //const [bloquecpostgrados, setbloquecpostgrados] = useState([]);
   // const [bloquecdeportivo, setbloquecdeportivo] = useState([]);
    //const [bloquendefinido, setbloquendefinido] = useState([]);
    ...
    const ErrorRed=()=>{
        Alert.alert(
            "Consulta fallida",
            "No es posible obtener la lista de aulas. Por favor intente mas tarde",
            [{text:'OK',style:'ok',}]
        )
    };
    

    // funcion para separar las aulas por bloques 
    const modificar=(totales)=>{
        var objtotal = totales;
        setbloquea(objtotal.filter(obj => obj.lug_bloque == 1));
        setbloqueb(objtotal.filter(obj => obj.lug_bloque == 2));
        setbloquec(objtotal.filter(obj => obj.lug_bloque == 3));
       // setbloqued(objtotal.filter(obj => obj.lug_bloque == 4));
        setbloquee(objtotal.filter(obj => obj.lug_bloque == 5));
        setbloquechec(objtotal.filter(obj => obj.lug_bloque == 6));
        setbloquecasita(objtotal.filter(obj => obj.lug_bloque == 7));
        setbloquearquite(objtotal.filter(obj => obj.lug_bloque == 8));
        //setbloquecpostgrados(objtotal.filter(obj => obj.lug_bloque == 9)); // en los bloques comentados no hay salones registrados
        //setbloquecdeportivo(objtotal.filter(obj => obj.lug_bloque == 10));
       // setbloquendefinido(objtotal.filter(obj => obj.lug_bloque == -1));
    };

    // llamada al api rest para traer la lista de laboratorios 
    useEffect(()=>{
        const url = 'https://reservapp.ucm.edu.co/api/1.0/list_aulas/'+rol+'/'
        fetch(url, {
            method: 'GET',
            headers: {
...
// mensajes de alerta 
    ...
    // funcion para convertir string de horas 
    ...
    return (
        <View style={styles.container}>
              ...
              <Text style={styles.textHeader}>Inicio/Aulas</Text>
              
                <ScrollView style={styles.scrollview}>

                    {(bloquea == 0 && bloqueb == 0 && bloquec == 0 && bloquee == 0 && bloquechec == 0 && bloquecasita == 0 && bloquearquite == 0)? <NoDisponible espacio={'aulas'}/>:
                        <>
                            {bloquea.length != 0 && <Text style={styles.encabezado}> Bloque A</Text>}
                            {bloquea.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible}/>
                            ))}
                            {bloqueb.length != 0 && <Text style={styles.encabezado}> Bloque B</Text>}
                            {bloqueb.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                            {bloquec.length != 0 && <Text style={styles.encabezado}> Bloque C</Text>}
                            {bloquec.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                            {bloquee.length != 0 && <Text style={styles.encabezado}> Bloque E</Text>}
                            {bloquee.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                            {bloquechec.length != 0 && <Text style={styles.encabezado}>  Bloque F</Text>}
                            {bloquechec.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                            {bloquecasita.length != 0 && <Text style={styles.encabezado}> Bloque G</Text>}
                            {bloquecasita.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                            {bloquearquite.length != 0 && <Text style={styles.encabezado}> Bloque H</Text>}
                            {bloquearquite.map((item,index)=>(
                                <ListAulas aula={item.lug_nombre}  codigo={item.lug_codigo_siga} piso={item.lug_piso} bloque={item.lug_bloque} lugar={item.lug_id} key={index} contador ={index+1} sillas={item.lug_numero_sillas} setNombreaula={setNombreaula} setBloque={setBloque} setPiso={setPiso} setLugar={setLugar} setCapacidad={setCapacidad} setCodsiga={setCodsiga} setModalVisible={setModalVisible} />
                            ))}
                        </> 
                    }
                ...
                <View style={styles.containerModal}>
                    <View style={styles.Moldal}>
                        ...
                        <View style={styles.header}>
                            ...
                            <Text style={styles.text}>{nombreaula} </Text> 
                ...
                <View style={styles.containerModal}>
                    <View style={styles.Moldal}>
                        ...
                        <View style={styles.resumen}>
                            ...
                            <ResumenReserva setHoraIni={setHoraIni} setHoraFin={setHoraFin}  auditorio={nombreaula}/>
                            <View >
...
const styles = StyleSheet.create({
    ...
    encabezado:{
        ...
        backgroundColor:'#92A3AD',
        fontFamily:'FiraSans-Light',
        color:"#FFFF",
        fontWeight:'normal',
        fontSize:0.08*dwidth,
